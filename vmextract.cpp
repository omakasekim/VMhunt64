...
struct Func {
     ADDR64 callAddr;              // same as the startAddr in FuncBody
...
map<ADDR64, list<FuncBody *> *> *
buildFuncList(list<Inst> *L)
{
     map<ADDR64, list<FuncBody *> *> *funcmap =
          new map<ADDR64, list<FuncBody *> *>;
     ...
     for (list<Inst>::iterator it = L->begin(); it != L->end(); ++it) {
          ...
          if (it->opcstr == "call") {
               ...
               map<ADDR64, list<FuncBody *> *>::iterator i = funcmap->find(it->addrn);
               if (i == funcmap->end()) {
                    ADDR64 calladdr = stoul(it->oprs[0], nullptr, 16);
                    funcmap->insert(pair<ADDR64, list<FuncBody *> *>(calladdr, NULL));
               }
...
void printFuncmap(map<ADDR64, list<FuncBody *> *> *funcmap)
{
     map<ADDR64, list<FuncBody *> *>::iterator it;
...
struct BB {
     ...
     BB() {}
     BB(ADDR64 begin, ADDR64 end);
     BB(ADDR64 begin, ADDR64 end, int type);
};

BB::BB(ADDR64 begin, ADDR64 end) : ty(0) {
     beginaddr = begin;
     endaddr = end;
}

BB::BB(ADDR64 begin, ADDR64 end, int type) {
     beginaddr = begin;
     endaddr = end;
     ty = type;
}
...
struct Edge {
     ...
     Edge() {}
     Edge(ADDR64 addr1, ADDR64 addr2, int type, int num);
};

Edge::Edge(ADDR64 addr1, ADDR64 addr2, int type, int num)
{
     fromaddr = addr1;
     toaddr = addr2;
     ty = type;
...
CFG::CFG(list<Inst> *L)
{
     ...
     ADDR64 addr1, addr2;     // addr1 is start and addr2 is end
     it = L->begin();
     addr1 = it->addrn;
     while (it != L->end()) {
...
void CFG::checkConsist()
{
     ...
     for (int i = 0, max = bbs.size(); i < max; ++i) {
          ADDR64 addr1, addr2, addr3, addr4;
